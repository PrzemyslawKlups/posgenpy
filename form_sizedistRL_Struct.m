function [sizedistRL_Struct] = form_sizedistRL_Struct(dinitial,dstep,dfinal,inputfolder,plot_f)
%form_sizedistRL_Struct reads the size dist files produced by a relabelled cluster sweep
%and forms a data structure with the data.
...
for i = dinitial:dstep:dfinal
    sizedistfile = [inputfolder '\sizedist-RL-dmax-' num2str(i) '.txt'];
    ...
    sz = size(C{1,1},1);
    if sz>1
    sizedistRL_Struct.(name) = dlmread(sizedistfile,'',1,0);%combining all the data into datastruct%
    else
    sizedistRL_Struct.(name) = [2,0];%this is because it doesn't like ready files where no clusters were found%
    end
end 
for j=dinitial:dstep:dfinal %add zeros in where there are no cluster%
    ...
    a = sizedistRL_Struct.(name); %saving typing dataStruct everytime%
    ...
    sizedistRL_Struct.(name)=a0;%reforming data Structure%
end
if strcmp(plot_f,'Yes') == 1
    dlargest = ['dmax' num2str(dfinal*100)];
    maxcluster = size(sizedistRL_Struct.(dlargest),1);%largest cluster (for all values of dmax)%
    ...
    zlabel('frequency relabelled','fontsize',12)
