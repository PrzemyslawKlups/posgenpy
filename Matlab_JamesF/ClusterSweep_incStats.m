function [] = ClusterSweep_incStats(posfile,rangefile,ClusteredIons,BulkIons,dinitial,dstep,dfinal,inclusion,outputfolder)
...
%dmax using input limits. Inclusion can be done, ouput is size distribution
...
%inclusion test 'Yes' if inclusion is desired. Some input is required
...
%required a number combination of clustered and bulk ions that doesn't already exist%
...
%XML files can be edited with notepad++, just add or reomve lines of the
%same format as desired%

x = size(ClusteredIons,2);
y =size(BulkIons,2);
status = mkdir(outputfolder);
if exist(['MatlabClusterSweep_incStats_' num2str(x) 'CI_' num2str(y) 'BI.xml']) == 2 %does correct input xml file exist%
    disp('Input xml file exists')
else
    disp('Input xml file not found, please create file of correct format, see ClusterSweep_incStats.m')
    return
end
DOMnode = xmlread(['MatlabClusterSweep_incStats_' num2str(x) 'CI_' num2str(y) 'BI.xml']);%loads xml file which is already in correct input format%
...
for i = dinitial:dstep:dfinal %parameter sweep in dmax%
    dmax = num2str(i);
    sizedistfile = [outputfolder '\sizedist-dmax-' num2str(i) '.txt'];
    clusterstatsfile = [outputfolder '\clusterstats-dmax-' num2str(i) '.txt'];
    if inclusion == 'yes'
        dbulk = num2str(i/2);
    else
       dbulk ='0.0'; 
...
xmlwrite('posgeninput_ClusterSweep.xml',DOMnode);%write final xml fed to posgen%
!posgen.exe & ;
status = dos('posgen posgeninput_ClusterSweep.xml'); %excutes posgen%
end
end
